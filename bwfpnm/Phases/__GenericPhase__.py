...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  2 11:36:25 2017
...
class GenericPhase(Core):
    ...
    def __init__(self, network=None, components=[], **kwargs):
        ...
        self['prop.temperature'] = 293.15
        self['prop.pressure'] = 101325.0
    ...
    def __getitem__(self, key):
        ...
        if key not in self.keys():
            try:
                ...
                temp = self._interleave_data(key, sources=self._physics)
                if temp is not None:
                    return temp
                else:
                    raise Exception
                ...
                prop = key.split('.')
                if prop[0] in ['pore', 'throat']:
                    logger.debug(key+' not on Phase nor on Physics. \
                                 Load prop.'+prop[-1]+ ' instead')
                    return super().__getitem__('prop.'+prop[-1])
                elif prop[0] == 'prop':
                    logger.debug(key+' not on Phase nor on Physics. \
                                 Load pore.'+prop[-1]+ ' instead')
                    return super().__getitem__('pore.'+prop[-1])
        else:
    ...
    def set_component(self, phase, mode='add'):
        ...
        if mode == 'add':
            if phase.name in self.phases():
                ...
                phase.models.add(propname='prop.temperature',
                                 model=OpenPNM.Phases.models.misc.mixture_value)
                phase.models.add(propname='prop.pressure',
                                 model=OpenPNM.Phases.models.misc.mixture_value)
                # Move T and P models to beginning of regeneration order
                phase.models.reorder({'prop.temperature': 0, 'prop.pressure': 1})
        elif mode == 'remove':
            if phase.name in self.phases():
    ...
    def check_mixture_health(self):
        ...
        for comp in self._phases:
            try:
                mole_sum = mole_sum + comp['prop.mole_fraction']
            except:
                pass
        return mole_sum
