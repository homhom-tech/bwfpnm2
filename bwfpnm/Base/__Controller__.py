...
class Controller(dict):
    ...
    def _output_pandas(self, obj):
        d = {}
        n = sp.size(obj._pc)
        d['pc'] = obj._Pc
        d['lpc'] = obj._log_Pc
        d['saturation'] = obj._saturation
        d['moisture content'] = obj._moisturecontent
        ...
        try:
            d['permeability effective'] = obj._permeability
        except:
            d['permeability effective'] = sp.zeros(n)
        ...
        try:
            d['conductance'] = obj._conductance
        except:
            d['conductance'] = sp.zeros(n)

        df = DataFrame(data=d)
        return df

    def save_flow_csv(self, obj, filename=''):
        if filename == '':
            filename = obj.name + '.csv'
        else:
            filename = filename.split('.')[0] + '.csv'

        df = _output_pandas(obj)
        df.to_csv(filename, sep="\t")

    def save_flow_xls(self, obj, filename=''):
        if filename == '':
