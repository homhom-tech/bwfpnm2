...
class DrainagePercolation(GenericAlgorithm):
    r"""
    ...
    def run(self, inlets, outlets=[], npts=25, inv_points=None,
        ...
        if inv_points is None:
            min_p = sp.amin(self._t_cap) * 1.02  # nudge min_p down slightly
            max_p = sp.amax(self._t_cap) * 0.98  # bump max_p up slightly
            ...
            self._inv_points = -sp.logspace(sp.log10(-min_p),
                                            sp.log10(-max_p),
                                            self._npts)
            self._inv_points = self._inv_points[::-1]
        else:
