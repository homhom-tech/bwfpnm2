...
class DryingPercolation(GenericAlgorithm):
    r"""
    Simulates a drying experiment by looping through a list of
    capillary pressures.
    ...
    def __init__(self, invading_phase=None, defending_phase=None,
        ...
        """
        super(DryingPercolation, self).__init__(**kwargs)
        ...
        logger.debug("Create Drying Percolation Algorithm Object")
    ...
    def run(self,
        ...
        try:
            ...
            try:
                ...
                pass
                logger.error('Capillary pressure neither assigned to \
                    defending phase ' + self._phase_def.name + ' nor to \
                    invading phase ' + self._phase_inv.name)
        try:
            ...
            logger.error('Capillary pressure not assigned to invading phase '
                ...
                defending phase ' + self._phase_def.name + ' instead')
            try:
                self._tp_cap = self._phase_def['pore.'+self._p_cap]
            except:
                pass
                logger.error('Capillary pressure neither assigned to \
                    defending phase ' + self._phase_def.name + ' nor to \
    ...
    def _do_outer_iteration_stage(self):
        ...
        _p_inv_uniq = sp.unique(self._p_inv)
        _t_inv_uniq = sp.unique(self._t_inv)
        self._pt_inv = sp.around(sp.r_[self._t_inv, self._p_inv], 3)
        self._pt_inv_uniq = sp.unique(self._pt_inv)
        # Find invasion sequence values (to correspond with IP algorithm)
        # searchsorted cannot deal with descending order, so have to do trick
        self._p_seq = sp.searchsorted(_p_inv_uniq, self._p_inv, side='right')
        self._t_seq = sp.searchsorted(_t_inv_uniq, self._t_inv, side='right')
        self._p_seq_all = sp.searchsorted(self._pt_inv_uniq, self._p_inv,
                                          side='right')
        self._t_seq_all = sp.searchsorted(self._pt_inv_uniq, self._t_inv,
                                          side='right')

        self._p_seq = sp.size(_p_inv_uniq) - self._p_seq
        self._t_seq = sp.size(_t_inv_uniq) - self._t_seq
        self._p_seq_all = sp.size(self._pt_inv_uniq) - self._p_seq_all
        self._t_seq_all = sp.size(self._pt_inv_uniq) - self._t_seq_all
        ...
        for i in range(self._npts):
            ...
            v_vapour = (sum(self._net['pore.volume'][inv_pores]) +
                        sum(self._net['throat.volume'][inv_throats]))
            new_sat = v_vapour/v_total
            sat -= new_sat
            self['pore.inv_sat'][inv_pores] = sat
            self['throat.inv_sat'][inv_throats] = sat

    def _do_one_inner_iteration(self, inv_val, boundary=False):
        r"""
        ...
        Pinvaded[temp] = True
        pmask0 = Pinvaded.copy()     # possible-to-dry pores
        if self._AL:
        ...
        # pmask: which pores are connected to invading sources [bool]
        pmask1 = sp.in1d(clusters, inv_clusters)  # accessable pores
        pmask2 = self._p_inv == sp.inf            # water-filled pores
        pmask = pmask1*pmask2
        # if no boundary inlet is spcified
        if not boundary:
            pmask = pmask*pmask0

        #Store result of invasion step
        self._p_inv[pmask] = inv_val
        ...
        tmask1 = (pmask1[temp[:, 0]] + pmask1[temp[:, 1]])*(Tinvaded)
        tmask2 = (self._t_inv == sp.inf)
        tmask = tmask1*tmask2
        self._t_inv[tmask] = inv_val
    ...
    def return_results(self, Pc=0, seq=None, sat=None,
        ...
        if(sat is not None):
            ...
            # definition: seq_i = (1 - sat_i)*n_seq
