# -*- coding: utf-8 -*-
"""
Created on Tue Aug 11 18:34:06 2015
...
def geometry(NetInstance, GeoClass=None, geoname='geo',
    ...
    if GeoClass is None:
        try:
            GeoClass = bpnm.Geometry.Pore_Throat
        except:
...
def algorithm(NetIns, WaterIns, VapourIns, AdsorpClass=None, DesorpClass=None,
    ...
    inv_sites = NetIns['pore.inlet'] + NetIns['pore.outlet']
...
def moist_content(geo, alg_wp, water_density,
    ...
    try:
        ...
        print('error: either volume of bulk material or porosity is required!')
        return
...
def permeability_bwf(pn, alg_wp, alg_dp, alg_flow_wp, alg_flow_dp,
    ...
    Pc_max = -1  # sp.amax(alg_wp._inv_points)
...
#        # calc effective flow
#        alg_flow_wp.calc_eff_flow_conceptual(
#            conductance=phys_moisture['throat.conduit_conductance_wp'])
        ctrl.purge_object(alg_flow_wp)
